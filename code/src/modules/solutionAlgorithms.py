...
class SolutionAlgorithms:
    ...
    def performApproxVI(self, df: pd.DataFrame, states: Dict[str, State], p: Probabilities, iniState: str, n: int, samples: int = 20):
        ...
        best_dec["value"] = best_dec["s_key"].apply(lambda s: states[s].get_V_N())
        #best_dec = best_dec.append(pd.DataFrame({
        #    "s_key": list(map(lambda s: s.getKey(), filter(lambda s: not s.getKey() in best_dec.s_key , states.values()))),
        #    "d_key": list(map(lambda s: ))
        #    "value": list(map(lambda s: s.get_V_N(), filter(lambda s: not s.getKey() in best_dec.s_key , states.values())))},
        #), ignore_index=True)
    ...
    def performApproximateDP(self):
        pass
        
        # Assign starting state
        # nicht alle decisions voret berechnen!
